...
def level_045():
    ...
    L.goal_on_path = True
    L.goal_region = (0.9, -4.0, 5.0, 9.0)
    L.tune_zones = {1: (0.3, 0.2)}
    L.spawn(-3.8, 7.8)
    slots = ground_with_recesses(L, -5, -2.8, -4.0, [-3.8])
    pit_spikes(L, -2.8, 5.0)
    ground_with_recesses(L, -2.2, 0.4, 0.6, [-0.4], recess_width=0.9, depth=0.8, h=1.2)
    L.block(0.65, 0.2, 0.5, 3.2, surface='Wall')
    L.wall(5.0, -4.0, 9.0)
    ...
    L.stars(UseAll(2, key='obj.l004.both'), Time(8))
...
def level_047():
    ...
    roll_start(L)
    ground_with_recesses(L, -3.4, 0.4, -4.0, [-1.4], recess_width=0.9, depth=0.5)
    pit_spikes(L, 0.4, 5.0)
    L.wall(5.0, -4.0, 9.0)
    zb = L.slot(-1.4, -4.15, [B])
...
def level_048():
    ...
    L.tune_zones = {1: (0.6, 0.6)}
    roll_start(L)
    ground_with_recesses(L, -3.4, 5.0, -4.0, [-1.6], recess_width=0.9, depth=0.5)
    L.block(0.6, -1.6, 0.5, 4.8, surface='Wall')
    L.platform(-0.6, 0.4, 1.6)
    L.button(-0.2, 0.7, emit=0, bid='b1')
    L.door(2.6, -2.6, channel=0, h=2.8)
    L.block(2.6, 2.4, 0.5, 7.2, surface='Wall')
    L.goal(4.0, -3.2, r=0.7)
    zb = L.slot(-1.6, -4.15, [B])
    zu = L.slot(-2.2, 2.6, [U])
    zu2 = L.slot(1.6, 2.6, [U])
    L.path_crystal(0.7)
    L.items(bomb=1, bumper=1)
    L.stars(Interaction('button', 1, key='obj.l036.button'), MaxObjects(2))
    L.solution((B, zb, 0), (U, zu, 0))
...
def level_049():
    ...
    L.tune_zones = {1: (0.6, 0.6)}
    roll_start(L)
    ground_with_recesses(L, -3.4, 0.6, -4.0, [-1.4], recess_width=0.9, depth=0.5)
    pit_spikes(L, 0.6, 5.0)
    L.obstacle(1.8, 3.6, 0.5, 3.0)
    L.wall(5.0, -4.0, 9.0)
    zb = L.slot(-1.4, -4.15, [B])
    zu = L.slot(1.2, 0.6, [U])
    zr = L.area(3.4, 1.6, 2.0, 1.0, ['ramp'], capacity=1, default_rot=0, min_rot=-45, max_rot=45)
    L.path_crystal(0.4)
    L.path_crystal(0.6)
    L.path_crystal(0.8)
...
def level_050():
    ...
    L.goal_on_path = True
    L.tune_zones = {1: (0.6, 0.6), 2: (0.6, 0.6)}
    roll_start(L)
    ground_with_recesses(L, -3.4, 0.0, -4.0, [-1.6], recess_width=0.9, depth=0.5)
    pit_spikes(L, 0.0, 5.0)
    L.obstacle(1.4, 3.0, 0.5, 12.0)
    L.wall(5.0, -4.0, 9.0)
    zb = L.slot(-1.6, -4.15, [B])
    zu1 = L.slot(-0.4, 2.2, [U])
    zu2 = L.slot(-3.8, 3.8, [U])
    za = L.slot(-1.8, 5.8, [P])
    zb2 = L.slot(3.2, 5.0, [P])
    L.path_crystal(0.3)
    L.path_crystal(0.55)
    L.path_crystal(0.85)
    L.items(bomb=1, bumper=2, portal_ab=1)
    L.stars(MaxObjects(4), AllCrystals())
    L.solution((B, zb, 0), (U, zu1, 0), (P, za, 0), (P, zb2, 180))
